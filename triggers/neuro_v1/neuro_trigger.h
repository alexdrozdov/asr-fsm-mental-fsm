...
public:
	...
	void Enter();
	float FannOutput(int i);

	int TryState(int nstate);  //Выполняет попытку перейти в указанное состояние. Возвращает новое состояние.
	//Может разрещить переход или отправить автомат в другое состояние в связи с невыполнением условия
...
extern int   neuro_v1_init_tcl(Tcl_Interp* interp);
extern int   neuro_v1_init_core(trig_init_environ* env);
extern void* neuro_v1_create(std::string filename);
